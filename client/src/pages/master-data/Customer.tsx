...
const customerFormSchema = z.object({
  ...
  creditLimitGroupId: z.coerce.number().optional(),
...
export default function Customer() {
  ...
  const getCompanyCodeName = (id: number) => {
    ...
    return companyCode ? `${companyCode.code} - ${companyCode.name}` : `Company Code ${id}`;
  };

  // Helper function to get credit limit group by ID
  const getCreditLimitGroup = (id: number) => {
    const creditLimitGroupsArray = Array.isArray(creditLimitGroups) ? creditLimitGroups : [];
    return creditLimitGroupsArray.find((group: any) => group.id === id);
  };

  // Auto-fill function when credit limit group is selected
  const handleCreditLimitGroupChange = (groupId: number) => {
    const group = getCreditLimitGroup(groupId);
    if (group) {
      addForm.setValue('creditLimitGroupId', groupId);
      addForm.setValue('creditLimit', parseFloat(group.creditLimit));
      addForm.setValue('currency', group.currency);
      addForm.setValue('paymentTerms', group.paymentTermsCode || '');
    }
  };
  ...
  const addCustomerMutation = useMutation({
    mutationFn: (data: z.infer<typeof customerFormSchema>) => {
      return fetch('/api/master-data/customer', {
        method: 'POST',
    ...
    onSuccess: (data) => {
      ...
      // Show customer details if needed
      if (data && data.id) {
        setSelectedCustomerId(data.id);
        setViewingCustomerDetails(data as Customer);
  ...
  const editCustomerMutation = useMutation({
    mutationFn: (data: { id: number; customer: z.infer<typeof customerFormSchema> }) => {
      return fetch(`/api/master-data/customer/${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteCustomerMutation = useMutation({
    mutationFn: (id: number) => {
      return fetch(`/api/master-data/customer/${id}`, {
        method: 'DELETE'
      }).then(res => res.json());
    },
    onSuccess: () => {
  ...
  const addContactMutation = useMutation({
    mutationFn: (data: z.infer<typeof contactFormSchema>) => {
      return fetch(`/api/master-data/customer/${selectedCustomerId}/contacts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(res => res.json());
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <Button 
            variant="outline" 
            onClick={() => {
              const csvContent = filteredCustomers?.map((customer: any) => ({
                Code: customer.code,
                Name: customer.name,
                Type: customer.type,
                Industry: customer.industry || '',
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <dl className="grid grid-cols-2 gap-x-4 gap-y-2 mt-3">
                        ...
                        <dd className="text-sm">
                          {getCompanyCodeName(viewingCustomerDetails.companyCodeId)}
                        </dd>
                      </dl>
                    </div>
                  ...
                  <TabsContent value="financial" className="space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Select the company code this customer belongs to
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Credit Limit Group</FormLabel>
                            <Select 
                              onValueChange={(value) => {
                                const groupId = parseInt(value);
                                handleCreditLimitGroupChange(groupId);
                              }} 
                            ...
                            >
                              ...
                              <SelectContent>
                                {creditLimitGroups.map((group: any) => (
                                  <SelectItem key={group.id} value={group.id.toString()}>
                                    {group.name} - {group.currency} {group.creditLimit}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          ...
                          <FormItem>
                            ...
                            <Select 
                              onValueChange={(value) => {
                                ...
                                const group = getCreditLimitGroup(groupId);
                                if (group) {
                                  editForm.setValue('creditLimitGroupId', groupId);
                                  editForm.setValue('creditLimit', parseFloat(group.creditLimit));
                                  editForm.setValue('currency', group.currency);
                                  editForm.setValue('paymentTerms', group.paymentTermsCode || '');
                                }
                              }} 
                              value={field.value ? field.value.toString() : ""}
                            >
