...
interface ReceivableConfirmation {
    id: number;
    fiscal_year: string;
    customer_id: number;
...
export default function ReceivableConfirmations() {
    const [selectedConfirmation, setSelectedConfirmation] = useState<ReceivableConfirmation | null>(null);
    ...
    // Fetch receivable confirmations
    const { data: confirmations, isLoading } = useQuery({
        queryKey: ['/api/finance/year-end/receivables', fiscalYear],
        queryFn: async () => {
            const response = await apiRequest(`/api/finance/year-end/receivables?fiscal_year=${fiscalYear}`);
            const data = await response.json();
            return data.data || [];
        },
    });
    ...
    const generateMutation = useMutation({
        mutationFn: async () => {
            const response = await apiRequest('/api/finance/year-end/receivables/generate', {
                method: 'POST',
        ...
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['/api/finance/year-end/receivables'] });
    ...
    const confirmMutation = useMutation({
        mutationFn: async (id: number) => {
            const response = await apiRequest(`/api/finance/year-end/receivables/${id}/confirm`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const disputeMutation = useMutation({
        mutationFn: async ({ id, reason, amount }: { id: number; reason: string; amount: string }) => {
            const response = await apiRequest(`/api/finance/year-end/receivables/${id}/dispute`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const sendLetterMutation = useMutation({
        mutationFn: async (id: number) => {
            const response = await apiRequest(`/api/finance/year-end/receivables/${id}/send-letter`, {
                method: 'POST',
            });
            return await response.json();
        },
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold">Receivable Balance Confirmations</h2>
                    <p className="text-gray-600">Confirm outstanding customer balances for year-end closing</p>
                </div>
                    ...
                    <Dialog open={showGenerateDialog} onOpenChange={setShowGenerateDialog}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Generate balance confirmations for all customers with outstanding balances
                                </DialogDescription>
                            </DialogHeader>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-yellow-600">
                            {confirmations?.filter((c: ReceivableConfirmation) => c.status === 'PENDING').length || 0}
                        </div>
                    </CardContent>
                </Card>
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-green-600">
                            {confirmations?.filter((c: ReceivableConfirmation) => c.status === 'CONFIRMED').length || 0}
                        </div>
                    </CardContent>
                </Card>
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-red-600">
                            {confirmations?.filter((c: ReceivableConfirmation) => c.status === 'DISPUTED').length || 0}
                        </div>
                    </CardContent>
                </Card>
            </div>
                ...
                <CardContent>
                    {isLoading ? (
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {confirmations.map((confirmation: ReceivableConfirmation) => (
                                    <TableRow key={confirmation.id}>
                                        <TableCell className="font-medium">{confirmation.customer_name}</TableCell>
                    ...
                    {selectedConfirmation && (
                        <div className="space-y-4">
                            <div className="bg-gray-50 p-4 rounded-lg">
                                ...
                                <p className="font-medium">{selectedConfirmation.customer_name}</p>
