...
const CompanyCodeExcelImport: React.FC = () => {
  ...
  const readExcelFile = (file: File): Promise<ImportedCompanyCode[]> =>
    new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const companyCodeData: ImportedCompanyCode[] = jsonData
            .map((row: any) => {
              const isActive =
              ...
              return {
                ...
                country: String(
                  row.country || row.Country || "United States"
                ).trim(),
                ...
                postalCode: String(
                  row.postalCode || row.PostalCode || row["Postal Code"] || ""
                ),
                phone: String(row.phone || row.Phone || ""),
                email: String(row.email || row.Email || ""),
                website: String(row.website || row.Website || ""),
                logoUrl: String(
                  row.logoUrl || row.LogoUrl || row["Logo URL"] || ""
                ),
                isActive,
              };
            })
  ...
  const handleImport = async (): Promise<void> => {
    ...
    try {
      const mappedData = previewData.map((companyCode) => ({
        code: companyCode.code,
        name: companyCode.name,
        description: companyCode.description || null,
        city: companyCode.city || null,
        country: companyCode.country,
        currency: companyCode.currency,
        taxId: companyCode.taxId || null,
        fiscalYear: companyCode.fiscalYear || null,
        address: companyCode.address || null,
        state: companyCode.state || null,
        postalCode: companyCode.postalCode || null,
        phone: companyCode.phone || null,
        email: companyCode.email || null,
        website: companyCode.website || null,
        logoUrl: companyCode.logoUrl || null,
        language: null as string | null,
        active:
          companyCode.isActive !== undefined ? companyCode.isActive : true,
      }));

      const response = await fetch("/api/master-data/company-code/bulk-import", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ companyCodes: mappedData }),
      });
      ...
      if (success > 0) {
        ...
        toast({
          ...
          description: `Successfully imported ${success} company codes${
        ...
        toast({
          title: "Import failed",
          description:
            "None of the company codes could be imported. Please check the errors.",
          variant: "destructive",
        });
      }
    } catch (error: any) {
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload an Excel file containing company code data to bulk import
          ...
          <div className="space-y-4">
              ...
              <div className="border rounded-md p-4">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="font-medium">
                    Preview: {previewData.length} Company Codes
                  </h3>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            {importResult && (
              ...
              >
                ...
                <AlertDescription>
                  <p>
                    Successfully imported {importResult.success} company codes.
                    {importResult.failed > 0 &&
                      ` Failed to import ${importResult.failed} company codes.`}
                  </p>
