...
async function addTaxJurisdictionToStates() {
  ...
  let client;
  try {
    console.log('🔄 Starting migration: Add tax_jurisdiction_id to states table...');
    ...
    if (columnCheck.rows.length > 0) {
      console.log('⚠️  Column tax_jurisdiction_id already exists in states table.');
    } else {
      ...
      await client.query(`
        CREATE INDEX IF NOT EXISTS idx_states_tax_jurisdiction_id 
        ON states (tax_jurisdiction_id);
      `);
